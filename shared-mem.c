...
shm
create_shm(const char* name, const size_t size)
{
...
map_shm(const shm x)
{
    AcReal* addr = (AcReal*)mmap(0, x.size, PROT_READ | PROT_WRITE, MAP_SHARED, x.fd, 0);
    if (addr == MAP_FAILED) {
        perror("mmap failed");
        close(x.fd);
        exit(EXIT_FAILURE);
...
allocate_shm(const int n_elems, const char* name) {
    ...
    const size_t size = n_elems*sizeof(AcReal);
    const shm shm_res  = create_shm(name, size);
    AcReal* res = map_shm(shm_res);
    //TP: create a semaphore with a same name intended for synchonized access to the shared mem file
    create_sem(name);
    close(shm_res.fd);
    return res;
}
